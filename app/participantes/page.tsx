...
export default function ParticipantesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Build a map from participante ID to user info
      const userMap = new Map<string, PbUser>();
      for (const u of users) {
        if (u.participante) userMap.set(u.participante, u);
      }
      ...
      const enriched = parts.map((p) => {
        ...
        return {
          ...
          usuarioAtivo: u ? !u.disabled : undefined,
        };
      });
  ...
  async function saveEdit() {
    ...
    try {
      await pb.collection("participantes").update(editing.id, {
        nome: editNome,
        celular: editCelular,
      });
            ...
            <tbody>
              {participantes.map((p) => (
                <tr key={p.id} className="border-b hover:bg-muted/30">
                  <td className="px-4 py-3 font-mono text-muted-foreground">{p.numeroCurso}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Dialog open={editing?.id === p.id} onOpenChange={(o) => !o && setEditing(null)}>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Editar Participante #{p.numeroCurso}</DialogTitle>
                        </DialogHeader>
